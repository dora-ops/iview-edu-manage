          ...
          <Modal v-model="addModal" width="600px" :closable="closeable" :mask-closable="maskCloseable">
            ...
            <div>
              <Form ref="formValidate" :model="addForm" :rules="ruleValidate" :label-width="80">
                <FormItem label="学员工号" prop="eva_stu_id">
                  <Input v-model="addForm.eva_stu_id" placeholder="学员工号"></Input>
                </FormItem>
              
                <FormItem label="就业公司" prop="eva_tea_content">
                  <Input type="textarea" v-model="addForm.eva_tea_content" placeholder="就业公司" ></Input>
                </FormItem>
               </Form>
            </div>
            <div slot="footer">
              ...
              <Form ref="formValidate1" :model="updateForm" :rules="ruleValidate" :label-width="80">
                <FormItem label="工号" prop="eva_tea_id">
                 ...
                 <FormItem label="评语" prop="eva_tea_content">
                  <TextArea v-model="addForm.eva_tea_content" placeholder="评语" style="width:514px"></TextArea>
                </FormItem>
              </Form>
            </div>
            <div slot="footer">
...
<script>
  ...
  export default {
    ...
    methods: {
      request(){
        this.$http.post('http://localhost:9090/auth/graduate/list', this.form)
          .then((res) => {
            if (res.data.success) {
      ...
      addTeacher(name){
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('http://localhost:9090/auth/graduate/add', this.addForm)
              .then((res) => {
                if (res.data.success) {
      ...
      updateTeacher(name){
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('http://localhost:9090/auth/evaluate/updateInfo', this.updateForm)
              .then((res) => {
                if (res.data.success) {
                  this.$Message.success(res.data.info);
                  this.cancelInput();
      ...
      deleteTeacher(index){
        ...
        this.$http.post('http://localhost:9090/auth/evaluate/delete', form)
          .then((res) => {
            if (res.data.success) {
              this.$Message.success(res.data.info);
