...
const getEvaluteList = async function() {
    const result = await teacherModel.getEvaluteList(this.request.body.eva_stu_id,this.request.body.role);
    const currentPage = this.request.body.currentPage;
    const pageSize = this.request.body.pageSize;
    let list = result.rows;
    let start = pageSize * (currentPage - 1);
...
//add
const addEvalute = async function() {
    const data = this.request.body;
    let tea = await teacherModel.addEvalute(data.eva_tea_id,data.eva_stu_id,data.eva_tea_content);
...
const delEvalute = async function(){
    const data = this.request.body;
    let res = await teacherModel.delEvalute(data.id);
